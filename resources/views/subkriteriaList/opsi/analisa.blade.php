...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
  <h1 class="h2">Prioritas Nilai</h1>
</div>
<div class="container">
    ...
    <table>
        ...
        <th scope="col" class="text-center">{{ $subkriteria[$i]['subctr_name'] }}</th>
        @endfor
        </thead>
        <tbody>
            <form action="/hitung-subkriteria/analisa/hitung/{{ $list->id }}" method="post" id="input-form">
                @csrf
                @for($i = 0; $i < count($subkriteria); $i++)
                <tr>
                <th scope="row">
                    {{ $subkriteria[$i]['subctr_name'] }}
                </th>
